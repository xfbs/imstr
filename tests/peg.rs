...
fn test_peg_easy() {
    ...
    assert_eq!(list_parser::list(&"[1,1,2,3,5,8]".into()), Ok(vec![1, 1, 2, 3, 5, 8]));
}
...
fn test_peg_medium() {
    ...
    assert_eq!(list_parser::list(&"[1,2,3]".into()), Ok(vec![Atom::Number("1".into()), Atom::Number("2".into()), Atom::Number("3".into())]));
}
